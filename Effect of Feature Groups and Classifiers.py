...
dataset = pd.read_excel(r"D:\Drug Target Interaction\Datasets\Main datasets\Experiments\All Dataset\Experiments\Enzyme Drug Target Pair Datasets\DrugBnak.xlsx")
...
clf_msvm = MSVM(probability=True)
pred_msvm = clf_msvm.fit(X_train, y_train).predict(X_test)
acc = accuracy_score(pred_msvm, y_test)
acc

msvm_pred_proba1 = clf_msvm.predict_proba(X_test)
msvm_pred_proba1 = msvm_pred_proba1[:, 1]
cm = confusion_matrix(pred_msvm, y_test)
cm
...
print(classification_report(pred_msvm, y_test))

sensitivity = (true_positive / (true_positive + false_negative))
specificity = (true_negative / (true_negative + false_positive))
sensitivity, specificity

auc_msvm_1 = roc_auc_score(y_test, msvm_pred_proba1)
print('AUC value of MSVM: %.4f' % auc_msvm_1)

f_measure = ((2 * true_positive)/ ((2 * true_positive) + false_positive + false_negative))
f_measure

mcc = matthews_corrcoef(pred_msvm, y_test)
mcc
...
clf_msvm = msvm(random_state=10, n_estimators=200, max_depth=10)
pred_msvm = clf_msvm.fit(X_train, y_train).predict(X_test)
acc = accuracy_score(pred_msvm, y_test)
acc

xgb_pred_proba1 = clf_msvm.predict_proba(X_test)
xgb_pred_proba1 = msvm_pred_proba1[:, 1]
...
fpr_1, tpr_1, _ = roc_curve(y_test, msvm_pred_proba1)
